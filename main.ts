...
async function handlePollButton (interaction: ButtonInteraction, args: string[]): Promise<void> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(codeBlock(updatedPoll.options.map((option, index) => `${String(index + 1).padStart(2, '0')}. ${option.padEnd(Math.max(...updatedPoll.options.map((o) => o.length)))} - [${updatedPoll.votes > 0 ? generatePercentageBar((updatedPoll.optionVotes[index] ?? 0 / updatedPoll.votes) * 100) : generatePercentageBar(0)}] - ${updatedPoll.optionVotes[index]} [${updatedPoll.votes > 0 ? ((updatedPoll.optionVotes[index] ?? 0 / updatedPoll.votes) * 100).toFixed(2).toString().padStart(5, '0') : '00'}%]`).join('\n')))
    .addFields({
      name: 'Гласови',
